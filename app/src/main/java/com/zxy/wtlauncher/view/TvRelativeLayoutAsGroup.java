...
import com.zxy.wtlauncher.util.TvUtil;
...
public class TvRelativeLayoutAsGroup extends RelativeLayout {
	...
	private OnChildLongClickListener onlongclickLinstener;
					...
					public void onFocusChange(final View child, boolean focus) {
						if (focus) {
							...
							if (onChildSelectListener != null) {
								onChildSelectListener.onChildSelect(child,true);
							}

						} else {
							if (onChildSelectListener != null) {
								onChildSelectListener.onChildSelect(child,false);
							}
							returnCover(child);
						}
					}
				...
				if(onlongclickLinstener!=null){
					child.setOnLongClickListener(new OnLongClickListener() {
						...
						public boolean onLongClick(View child) {
							onlongclickLinstener.onChildLongClick(child);
							return true;
						}
					});
				}
...
//			}
			...
			Log.i(VIEW_LOG_TAG, l + "--" + t + "--" + (r - l) + "--" + (b-t));
			Log.i(VIEW_LOG_TAG, item.getLeft() + "--" + item.getTop()+ "--" +item.getWidth()+"--"+item.getHeight());
			animator.addUpdateListener(new AnimatorUpdateListener() {
				...
				public void onAnimationUpdate(ValueAnimator valueAnimator) {
					...
					int width = mEvaluator.evaluate(fraction, cursor.getWidth(), r-l);
					int height = mEvaluator.evaluate(fraction, cursor.getHeight(), b-t);
					int leftMargin = mEvaluator.evaluate(fraction, cursor.getLeft(), l);
					int topMargin = mEvaluator.evaluate(fraction, cursor.getTop(), t);

					cursor.layout(leftMargin, topMargin, leftMargin+width, topMargin+height);
					requestLayout();
				}
			});
			animator.setDuration(durationTranslate*100);
...
//								.toString());
	...
	public void setOnLongClickListener(OnChildLongClickListener myListener) {
		this.onlongclickLinstener = myListener;
	}

	public interface OnChildSelectListener {
		public void onChildSelect(View child,boolean selected);
	}
